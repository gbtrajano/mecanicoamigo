...
export default function NotasFiscaisPage() {
  ...
  const [formData, setFormData] = useState({
    numero: '', tipo: 'NF-e' as 'NF-e' | 'NFC-e' | 'NFS-e',
    ordemId: '', clienteId: '', valor: 0, status: 'emitida' as 'emitida' | 'cancelada',
    chaveAcesso: ''
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setEditingNota(null); setFormData({ numero: '', tipo: 'NF-e', ordemId: '', clienteId: '', valor: 0, status: 'emitida', chaveAcesso: '' }); setModalOpen(true) }}
      ...
      <div className="bg-surface rounded-xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-text-muted text-sm">Nenhuma nota fiscal encontrada</td></tr>
              ) : (
                filtered.map(n => {
                  const cliente = clientes.find(c => c.id === n.clienteId)
                  return (
                    <tr key={n.id} className="border-b border-border hover:bg-bg/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => { setEditingNota(n); setFormData({ ...n }); setModalOpen(true) }} className="p-1.5 rounded-lg hover:bg-primary/10 text-text-muted hover:text-primary transition-colors">
                            <Edit className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(n.id)} className="p-1.5 rounded-lg hover:bg-danger/10 text-text-muted hover:text-danger transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={formData.tipo} onChange={e => setFormData({...formData, tipo: e.target.value as 'NF-e' | 'NFC-e' | 'NFS-e'})} required
                className="w-full px-3 py-2 rounded-lg border border-border bg-bg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
          ...
          <div>
            ...
            <select value={formData.ordemId} onChange={e => {
              ...
              {ordens.map(o => {
                ...
                return <option key={o.id} value={o.id}>{o.numero} - {cliente?.nome || 'Sem cliente'} (R$ {o.valorTotal.toFixed(2)})</option>
              })}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-text mb-1">Valor</label>
            <input type="number" step="0.01" value={formData.valor} onChange={e => setFormData({...formData, valor: Number(e.target.value)})} readOnly
          ...
          <div>
            ...
            <input value={formData.chaveAcesso} onChange={e => setFormData({...formData, chaveAcesso: e.target.value})}
              className="w-full px-3 py-2 rounded-lg border border-border bg-bg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
          </div>
          <div className="flex justify-end gap-3">
            ...
            <button type="submit" className="px-4 py-2 rounded-lg bg-primary hover:bg-primary-dark text-white text-sm">{editingNota ? 'Salvar' : 'Emitir'}</button>
          </div>
        </form>
      </Modal>
    </div>
